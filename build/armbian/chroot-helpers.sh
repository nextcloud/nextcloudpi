...
function mount_chroot() {
  ...
  # mount root onto itself to allow remounts from within chroot (docker-support)
	...
	# Expose host cgroups (docker-support)
	mount --bind /sys/fs/cgroup "${target}"/sys/fs/cgroup
...
function umount_chroot() {
	...
	while grep -Eq "${target}\/(dev|proc|sys|tmp|var\/tmp|run\/user\/0|)" /proc/mounts; do
		...
		umount "${target}"/sys/fs/cgroup || true
