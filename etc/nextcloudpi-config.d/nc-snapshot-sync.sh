...
configure()
{
  ...
  cat > /etc/cron.daily/ncp-btrfs-sync <<EOF
#!/bin/bash
/usr/local/bin/btrfs-sync -qd $ZIP $SNAPDIR_ $DESTINATION_
EOF
  chmod +x /etc/cron.daily/ncp-btrfs-sync
